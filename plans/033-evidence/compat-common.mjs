...
export const evidence = path.join(repo, "plans/033-evidence/compatibility");
export const raw = path.join(repo, ".yarn/simplification-evidence/033/compatibility");
export const dist = path.join(repo, ".yarn/simplification-evidence/033/artifacts/candidate/dist/index.mjs");
...
export async function identity() {
  ...
  const source = path.join(repo, ".yarn/.codex-worktrees/plan033/vite-plugin-react-docgen-typescript/packages/vite-plugin-react-docgen-typescript/src");
  return {
    evaluatedSha: sha,
...
export function observeExtractions() {
  ...
  parser.withCompilerOptions = (...args) => {
    ...
    const parse = instance.parseWithProgramProvider;
    instance.parseWithProgramProvider = (...input) => { count += 1; return parse(...input); };
    return instance;
  };
  return () => count;
}
