...
const freezeFiles = [__filename, path.join(evidence, 'runner-bulk.mjs'), path.join(raw, 'backend-library.mjs'), path.join(raw, 'storybook-runtime/package-lock.json'),
...
saveNew(path.join(evidence, 'bulk-freeze.json'), { createdAt: new Date().toISOString(), freeze, host: { node: process.version, platform: process.platform, cpu: os.cpus()[0].model, logicalCpus: os.cpus().length }, budget: { rounds: 3, processes: 9, timeoutMs: 120000 } });
const attempts = [];
for (let round = 0; round < 3; round++) {
  const order = [...modes.slice(round), ...modes.slice(0, round)];
  for (const [position, mode] of order.entries()) {
    const output = path.join(raw, `bulk-${round}-${position}-${mode}.json`);
    ...
    const child = cp.spawnSync(process.execPath, [path.join(evidence, 'runner-bulk.mjs'), mode, output, '188', '--cold-only'], { cwd: repo, encoding: 'utf8', windowsHide: true, timeout: 120000, maxBuffer: 4 * 1024 * 1024 });
    ...
    fs.writeFileSync(path.join(evidence, 'bulk-attempts.json'), JSON.stringify(attempts, null, 2) + '\n');
...
saveNew(path.join(evidence, 'bulk-comparison.json'), { status: 'passed', processes: attempts.length, parity: '188 complete normalized components match the independent oracle in every run', lanes });
