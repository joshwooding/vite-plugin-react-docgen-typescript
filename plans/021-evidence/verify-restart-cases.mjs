...
import { child, config, dist, equal, evidence, filesUnder, identity, json, metadata, modes, options, raw, removeFixture, writeJson } from "./common.mjs";
...
async function transform(manifest) {
  ...
  // Config watch registration occurs only on the persisted-hit branch in this direct probe.
  return { pid: process.pid, root, cache, metadata: metadata(result), persistedHit: watchFiles.includes(path.join(root, "tsconfig.json")), watchFiles };
}
