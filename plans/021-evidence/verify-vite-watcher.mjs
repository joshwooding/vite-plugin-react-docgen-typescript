...
import { child, config, dist, equal, evidence, filesUnder, identity, json, metadata, modes, options, raw, removeFixture, require, writeJson } from "./common.mjs";

const script = fileURLToPath(import.meta.url);
const windowMs = 5000;
const normalized = (f) => path.resolve(f).replaceAll("\\", "/");
const compilerLoaded = () => Boolean(require.cache[require.resolve("typescript")]);
...
async function observe(manifest) {
  ...
  const events = [];
  const hotHooks = [];
  const observer = { name: "plan021-observer", hotUpdate({ file, type }) { hotHooks.push({ file: normalized(file), type }); } };
  const loadedBeforeServer = compilerLoaded();
  ...
  try {
    ...
    const loadedAfterInitial = compilerLoaded();
    const initialRegisteredWatchFiles = [...registeredWatchFiles];
    if (action === "seed") return { pid: process.pid, root, initial, loadedBeforeServer, loadedAfterInitial };
    const persistedHit = initialRegisteredWatchFiles.includes(normalized(path.join(root, "tsconfig.json")));
    if (cache && (!persistedHit || loadedAfterInitial)) throw new Error("Persistent startup was not observably cache-only");
    ...
    return {
      ...
      loadedBeforeServer, loadedAfterInitial, persistedHit, initialRegisteredWatchFiles,
      windowMs,
...
async function main() {
  ...
  const rows = [];
  for (const mode of modes) for (const startup of ["fresh", "persistent-only"]) {
    ...
    try {
      ...
      const cache = startup === "persistent-only";
