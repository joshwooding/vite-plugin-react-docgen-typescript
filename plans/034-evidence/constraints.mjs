...
const output = path.join(repo, remaining ? 'plans/034-evidence/constraints-remaining-results.json' : 'plans/034-evidence/constraints-results.json');
...
const control = async (state,f) => {
  await state.server.transformRequest('/' + normalizePath(path.relative(f.root,f.control))); 
...
const rows=[];
for(const kind of (remaining ? ['depth','near-consecutive','offline-delete'] : ['ignored','disabled','scope','depth','near-consecutive','offline-delete'])) {
  const f=fixture();const row={kind,mode:'legacy'};let state;
  if(kind==='depth') { f.control=path.join(f.root,'control.js');writeFileSync(f.control,'export const control=1; if(import.meta.hot) import.meta.hot.accept();\n'); }
  try {
