...
const resolvePluginEntry = (environment) => {
  const directory = path.join(environment, "node_modules/@joshwooding/vite-plugin-react-docgen-typescript");
  const manifest = json(path.join(directory, "package.json"));
  assert.equal(manifest.exports["."].import, "./dist/index.mjs");
  return path.resolve(directory, manifest.exports["."].import);
};
...
if (operation === "oracle") {
  ...
  const { default: createPlugin } = await import(pathToFileURL(resolvePluginEntry(input.environment)).href);
  ...
  const pluginEntry = resolvePluginEntry(environment);
