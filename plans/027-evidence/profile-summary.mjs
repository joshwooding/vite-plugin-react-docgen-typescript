...
function owner(frame) {
  ...
  if (
    url.endsWith("/packages/vite-plugin-react-docgen-typescript/dist/index.mjs")
  )
    return "Plugin";
  if (url.startsWith("node:fs") || url.includes("internal/fs"))
    return "Node filesystem";
  if (url.includes("/plans/027-evidence/")) return "Profile driver";
...
for (const mode of modes)
  for (const phase of ["cold", "shared"]) {
    const report = json(
      path.join(evidence, "profiles", mode + "-profile-both.json"),
    );
