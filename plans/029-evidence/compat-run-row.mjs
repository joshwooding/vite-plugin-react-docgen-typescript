...
const expected = JSON.parse(
  readFileSync(path.join(evidence, "compatibility/artifact.json"), "utf8"),
);
...
const output = path.join(evidence, "compatibility/rows");
mkdirSync(output, { recursive: true });
const label = String(rowIndex + 1).padStart(2, "0");
const installedFile = path.join(output, `${label}-installed.json`);
for (const suffix of [".json", "-installed.json", "-stdout.txt", "-stderr.txt"])
  assert(
    !existsSync(path.join(output, `${label}${suffix}`)),
    "Preserve existing attempts before rerunning a row",
  );
const args = [
  "--import",
  pathToFileURL(path.join(evidence, "compat-observe-install.mjs")).href,
  verifier,
  "--package",
  packageArchive,
  "--typescript",
...
const result = spawnSync(process.execPath, args, {
  ...
  windowsHide: true,
  timeout: 300_000,
  env: { ...process.env, PLAN029_DEPENDENCY_RECORD: installedFile },
});
